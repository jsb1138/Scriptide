...
export async function createMeeting(
  ...
  const joinInfo: any = await API.graphql(
    graphqlOperation(createChimeMeeting, {
      title: title,
      name: attendeeName,
      region: region,
    })
...
export async function joinMeeting(meetingId: string, name: string) {
  const joinInfo: any = await API.graphql(
    graphqlOperation(joinChimeMeeting, { meetingId: meetingId, name: name })
  );
...
export async function endMeeting(meetingId: string) {
  const endInfo: any = await API.graphql(
    graphqlOperation(endChimeMeeting, { meetingId: meetingId })
  );
  const endInfoJson = endInfo.data.endChimeMeeting;
  await API.graphql(
    graphqlOperation(deleteMeetingGraphQL, { input: { title: meetingId } })
  );
  return endInfoJson;
}

export async function addMeetingToDB(
  title: string,
  meetingId: string,
  meetingData: string
) {
  await API.graphql(
    graphqlOperation(createMeetingGraphQL, {
      input: { title: title, meetingId: meetingId, data: meetingData },
    })
  );
}
...
export async function addAttendeeToDB(
  ...
  await API.graphql(
    graphqlOperation(createAttendeeGraphQL, {
      input: { attendeeId: attendeeID, name: attendeeName },
    })
  );
}

export async function getMeetingFromDB(title: string) {
  const meetingInfo = await API.graphql(
    graphqlOperation(getMeeting, { title: title })
  );
  return meetingInfo;
}
